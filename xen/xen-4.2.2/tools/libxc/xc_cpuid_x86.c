...
static void cpuid_hook(uint32_t idx, uint32_t count, uint32_t *regs, int pv_context)
{
#ifdef __i386__
    ...
    asm volatile (
        ...
        : : "a" (idx), "b" (count), "c" (pv_context), "S" (regs) : "memory" );
#else
    asm volatile (
        "test %5,%5 ; jz 1f ; ud2a ; .ascii \"xen\" ; 1: cpuid\n\t"
        : "=a" (regs[0]), "=b" (regs[1]), "=c" (regs[2]), "=d" (regs[3])
        : "0" (idx), "1" (pv_context), "2" (count) );
#endif
}
...
static void cpuid(const unsigned int *input, unsigned int *regs)
{
    ...
    cpuid_hook(input[0], count, regs, 1);
