...
extern void do_handle_hyperv(struct cpu_user_regs *regs);
...
//    if(this_cpu(curr_vcpu)->processor == 0 && irq < 0 && vector == 0x37) {
////    	if(this_cpu(curr_vcpu)->domain->domain_id != 0 || smp_processor_id() != 0) {
////    		printk("got a hyper-v IRQ, but domain_id=%d, cpuid=%d\n", this_cpu(curr_vcpu)->domain->domain_id, smp_processor_id());
////    	}
//        irq = 7;
//        //raise_softirq(0x7);
//        //cpu_raise_softirq(0, 0x7);
//        //goto out_no_unlock;
//    }
//    else
    if (irq < 0) {
    ...
    if (likely(desc->status & IRQ_GUEST) )
    {
        ...
        {
            s_time_t now = NOW();
            printk("irq_ratelimit_timer is being used!!!, irq=0x%x, vector=0x%x \n", irq, vector);
