...

{
    ...
    case 0x70000000:
    {
    	...
    	temp = (struct hyper_call_param *)a.value;

    	//printk("=====XEN-BLANKET=====domain0 hyper-v hypercall, control=0x%lx, input=0x%p, output=0x%p, hypercall_page=0x%p, status=0x%lx\n",
    	//			temp->control, temp->input, temp->output, temp->hypercall_page, temp->hv_status);

    	__asm__ __volatile__("mov %0, %%r8" : : "r" (temp->output) : "r8");
		__asm__ __volatile__("call *%3" : "=a" (temp->hv_status) :
					 "c" (temp->control), "d" (temp->input),
					 "m" (temp->hypercall_page));

		//printk("=====XEN-BLANKET=====domain0 hyper-v hypercall finished \n");
