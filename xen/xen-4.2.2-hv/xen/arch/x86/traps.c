...
int rdmsr_hypervisor_regs(uint32_t idx, uint64_t *val)
{
    ...
    //	printk("=====XEN-BLANKET=====rdmsr_hypervisor_regs: domainid=%d, base=0x%x, idx=0x%x      \n", d->domain_id, base, idx);

    if ( current->domain->domain_id == 0 ) {
		if ( rdmsr_safe(idx, temp) )
			return 0;
		*val = temp;
...
int wrmsr_hypervisor_regs(uint32_t idx, uint64_t val)
{
    ...
    //printk("=====XEN-BLANKET=====wrmsr_hypervisor_regs: domainid=%d, base=0x%x, idx=0x%x      \n", d->domain_id, base, idx);

    if ( current->domain->domain_id == 0 ) {
		if ( wrmsr_safe(idx, val) != 0 ) {
			return 0;
		}
...
static void pv_cpuid(struct cpu_user_regs *regs)
{
    ...
    if ( current->domain->domain_id != 0 )
    {
        ...
        //printk("=====XEN-BLANKET=====domainU CPUID is called, EAX=0x%lx, EBX=0x%lx, ECX=0x%lx, EDX=0x%lx\n", regs->eax, regs->ebx, regs->ecx, regs->edx);

        if ( !cpuid_hypervisor_leaves(a, c, &a, &b, &c, &d) )
            domain_cpuid(current->domain, a, c, &a, &b, &c, &d);
        //if(cpuid_leaf == 0x1 || cpuid_leaf == 0x80000001 || cpuid_leaf == 0xd || cpuid_leaf == 0x7)
	    // printk("=====XEN-BLANKET=====domainU CPUID is called, EAX=0x%x, ECX=0x%x\n", cpuid_leaf, sub_leaf);
        switch ( cpuid_leaf )
        {
        case 0xd:
        {
	...
	//if(regs->eax == 0x1 || regs->eax == 0x80000001 || regs->eax == 0xd || regs->eax == 0x7)
	//{
		//printk("=====XEN-BLANKET=====domain0 CPUID is called, EAX=0x%lx, EBX=0x%lx, ECX=0x%lx, EDX=0x%lx\n", regs->eax, regs->ebx, regs->ecx, regs->edx);
		//printk("=====XEN-BLANKET=====Hardware features: eax:0x%x, ebx:0x%x, ecx:0x%x, edx:0x%x\n", a, b, c, d);
	//}
 ...
 out:
    //if((uint32_t)regs->eax != 0x70000000)
    //if(regs->eax == 0x1 || regs->eax == 0x80000001 || regs->eax == 0xd || regs->eax == 0x7)
        //printk("=====XEN-BLANKET=====Return: eax:0x%x, ebx:0x%x, ecx:0x%x, edx:0x%x\n", a, b, c, d);
    regs->eax = a;
    regs->ebx = b;
    regs->ecx = c;
    regs->edx = d;
...
/*        case HV_X64_MSR_GUEST_OS_ID:
        ...
        case HV_X64_MSR_SINT15:                       
            //gdprintk(XENLOG_WARNING, "Received Hyper-V WRMSR: 0x%p, content:0x%016"PRIx64"\n,",
             //           _p(regs->ecx), msr_content);
            if ( wrmsr_safe(regs->ecx, msr_content) != 0 )
                goto fail;
            break;*/
        default:
...
/*        case HV_X64_MSR_GUEST_OS_ID:
        ...
        case HV_X64_MSR_SINT14:                       
        case HV_X64_MSR_SINT15:                       
            //gdprintk(XENLOG_WARNING, "Received Hyper-V RDMSR: 0x%p, content:0x%016"PRIx64"\n,",
            //            _p(regs->ecx), msr_content);
            goto rdmsr_normal;
            break;    
        */
...
long do_set_trap_table(XEN_GUEST_HANDLE(const_trap_info_t) traps)
{
    ...
    for ( ; ; )
    {
        ...
        //printk("=====XEN-BLANKET=====do_set_trap_table: cur.vector = %x\n", cur.vector);
