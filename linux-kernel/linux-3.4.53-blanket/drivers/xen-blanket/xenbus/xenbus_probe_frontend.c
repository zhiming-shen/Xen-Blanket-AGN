...
{
	//printk("XEN-BLANKET: Debug Otherend Changed.\n");
...
static int is_device_connecting(struct device *dev, void *data, bool ignore_nonessential)
{
 ...
 // printk("XEN_BLANKET: DEBUG: calling is_device_connecting\n");
	/*
...
static void wait_for_devices(struct xenbus_driver *xendrv)
{
	...
	while (exists_non_essential_connecting_device(drv))
		if (wait_loop(start, 10, &seconds_waited))
			break;

	/* Skips PVKB and PVFB check.*/
	while (exists_essential_connecting_device(drv))
		if (wait_loop(start, 20, &seconds_waited))
			break;
...
{
	xenbus_scanf_hvm(XBT_NIL, v[XS_WATCH_PATH], "", "%i", &backend_state);
	printk(KERN_DEBUG "XENBUS: backend %s %s\n",
			v[XS_WATCH_PATH], xenbus_strstate_hvm(backend_state));
