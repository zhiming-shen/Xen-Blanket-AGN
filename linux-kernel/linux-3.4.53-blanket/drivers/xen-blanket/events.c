...
static int find_virq(unsigned int virq, unsigned int cpu)
{
	...
	for (port = 0; port <= NR_EVENT_CHANNELS; port++) {
		...
		status.port = port;
		rc = HYPERVISOR_blanket_event_channel_op(EVTCHNOP_status, &status); //Notes Qin:changed by Qin, because there is no find_virq in orginal file.
		if (rc < 0)
			continue;
...
EXPORT_SYMBOL_GPL(unbind_from_irqhandler_hvm); //Notes Qin: below there is no such function in 3.1.2
...
static int rebind_irq_to_cpu_hvm(unsigned irq, unsigned tcpu)
{
			    ...
			    bool force)
{
	...
	return rebind_irq_to_cpu_hvm(data->irq, tcpu);
}
...
//	}  //Notes from Qin: this block is different from version 3.1.2
