	...
	if (scsi_sg_count(scmnd)) {
		...
		for (i = 0; i < sg_count; i++)
			cmd_request->data_buffer.pfn_array[i] =
				pfn_to_mfn(page_to_pfn(sg_page((&sgl[i]))));
#else
		for (i = 0; i < sg_count; i++)
			cmd_request->data_buffer.pfn_array[i] =
		...
		cmd_request->data_buffer.offset =
				arbitrary_virt_to_machine(scsi_sglist(scmnd)).maddr & (PAGE_SIZE-1);
		cmd_request->data_buffer.pfn_array[0] =
				arbitrary_virt_to_machine(scsi_sglist(scmnd)).maddr >> PAGE_SHIFT;
#else
		cmd_request->data_buffer.offset =
