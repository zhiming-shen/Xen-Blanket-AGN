				  ...
				  struct rndis_request *req)
{
	...
	packet->page_buf[0].pfn = arbitrary_virt_to_machine(&req->request_msg).maddr >>
					PAGE_SHIFT;
#else
			     ...
			     struct hv_netvsc_packet *pkt)
{
	...
	pkt->is_data_pkt = true;
#if XENBLANKET_ENABLED
	pkt->page_buf[0].pfn = arbitrary_virt_to_machine(rndis_msg).maddr >> PAGE_SHIFT;
#else
...
#if XENBLANKET_ENABLED
		pkt->page_buf[1].pfn = arbitrary_virt_to_machine((void *)((ulong)
			rndis_msg + pkt->page_buf[0].len)).maddr >> PAGE_SHIFT;
#else
